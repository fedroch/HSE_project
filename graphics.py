...
def plot_training_loss(epoch_losses, step_interval=50, save_name='training_loss.png'):
    '''график тренировочного loss по шагам (синие оттенки на белом фоне)'''
    fig, ax = plt.subplots(figsize=(11, 5))
    fig.patch.set_alpha(0)
    ax.set_facecolor('none')

    blues = plt.cm.Blues(np.linspace(0.45, 0.95, len(epoch_losses)))
    steps_per_epoch = max(len(e) for e in epoch_losses) * step_interval

    for i, losses in enumerate(epoch_losses):
        x = np.arange(1, len(losses) + 1) * step_interval + i * steps_per_epoch
        ax.plot(x, losses, color=blues[i], linewidth=2)
        if i > 0:
            ax.axvline(i * steps_per_epoch, color='#9ecae1',
                       linestyle='--', linewidth=1, alpha=0.7)
    ax.set_title('Тренировочный loss', fontsize=13, color='#08306b')
    ax.set_xlabel('Шаг обучения')
    ax.set_ylabel('Loss')
    fig.tight_layout()
    fig.savefig(FIGURES_DIR / save_name, dpi=150, bbox_inches='tight', transparent=True)
    print(f"График сохранён: {FIGURES_DIR / save_name}")
    plt.close(fig)
